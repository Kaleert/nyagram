...
import pro.kaleert.nyagram.context.BotContextHolder;
...
public class NyagramExecutor implements Executor {
    ...
    private Runnable wrapWithContext(Runnable task, Long shardKey, int shardIndex) {
        var commandContext = BotContextHolder.getContext();
        var mdcMap = MDC.getCopyOfContextMap();
        
        String currentToken = pro.kaleert.nyagram.context.BotTokenContext.getToken();
        String currentUsername = pro.kaleert.nyagram.context.BotTokenContext.getUsername();

        return () -> {
            if (commandContext != null) BotContextHolder.setContext(commandContext);
            if (mdcMap != null) MDC.setContextMap(mdcMap);
            if (currentToken != null) pro.kaleert.nyagram.context.BotTokenContext.setToken(currentToken);
            if (currentUsername != null) pro.kaleert.nyagram.context.BotTokenContext.setUsername(currentUsername);
            ...
            try {
                ...
                MDC.clear();
                pro.kaleert.nyagram.context.BotTokenContext.clear();
            }
        };
    }
