 ...
 * Используется в {@link pro.kaleert.nyagram.middleware.FsmMiddleware} для маршрутизации.
 * </p>
 *
 * @since 1.0.0
 */
...
public class FsmRegistry implements BeanPostProcessor { // <-- Реализуем этот интерфейс
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) {
        ...
        pro.kaleert.nyagram.fsm.wizard.Scene sceneAnn = AnnotatedElementUtils.findMergedAnnotation(targetClass, pro.kaleert.nyagram.fsm.wizard.Scene.class);
        if (sceneAnn != null) {
            String sceneName = sceneAnn.value();
            for (Method method : targetClass.getDeclaredMethods()) {
                pro.kaleert.nyagram.fsm.wizard.Step stepAnn = AnnotatedElementUtils.findMergedAnnotation(method, pro.kaleert.nyagram.fsm.wizard.Step.class);
                if (stepAnn != null) {
