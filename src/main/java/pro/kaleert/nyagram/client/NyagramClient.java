package pro.kaleert.nyagram.client;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import pro.kaleert.nyagram.api.dto.TelegramResponse;
import pro.kaleert.nyagram.api.exception.TelegramApiException;
import pro.kaleert.nyagram.exception.TelegramNetworkException;
...
import pro.kaleert.nyagram.api.objects.File;
import pro.kaleert.nyagram.api.methods.GetFile;
import pro.kaleert.nyagram.client.retry.ExponentialBackoffStrategy;
...
public class NyagramClient {
     ...
     * </p>
     *
     * @param method Объект метода API (например, {@link pro.kaleert.nyagram.api.methods.send.SendMessage}).
     * @param <T> Тип возвращаемого значения (определяется методом).
     * @return Результат выполнения запроса (объект Telegram).
     * @throws pro.kaleert.nyagram.api.exception.TelegramApiRequestException если API вернул ошибку.
     */
    ...
    private <T extends Serializable> T executeInternal(BotApiMethod<T> method, String baseUrl) {
        // 🪄 МАГИЯ MULTI-BOT: Сначала ищем токен в контексте потока, если нет - берем из конфига
        String dynamicToken = pro.kaleert.nyagram.context.BotTokenContext.getToken();
        String currentToken = (dynamicToken != null && !dynamicToken.isBlank()) 
                ? dynamicToken 
                : botConfig.getBotToken();

        if (currentToken == null || currentToken.isBlank()) {
            throw new IllegalStateException("Bot token is missing! Cannot execute " + method.getMethod());
        }

        String uri = String.format("%s/bot%s/%s", baseUrl, currentToken, method.getMethod());
    ...
    private boolean requiresMultipart(BotApiMethod<?> method) {
        ...
        if (method instanceof pro.kaleert.nyagram.api.methods.send.SendMediaGroup) {
            return true; 
        }
     ...
     * <p>
     * Путь к файлу можно получить, вызвав {@link pro.kaleert.nyagram.api.methods.GetFile}.
     * </p>
     *
