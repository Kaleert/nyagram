...
import pro.kaleert.nyagram.api.methods.updatingmessages.DeleteMessage;
...
import pro.kaleert.nyagram.api.objects.message.MaybeInaccessibleMessage;
...
import pro.kaleert.nyagram.i18n.LocaleService;
import pro.kaleert.nyagram.i18n.LocaleResolver;
import pro.kaleert.nyagram.i18n.TranslationResolver;
...
/**
    ...
    public void react(String emoji) {
        getMessage().ifPresent(msg -> {
            try {
                client.execute(pro.kaleert.nyagram.api.methods.reactions.SetMessageReaction.builder()
                        .chatId(msg.getChat().getId().toString())
                        .messageId(msg.getMessageId().intValue())
                        .reaction(java.util.List.of(
                                pro.kaleert.nyagram.api.objects.reactions.ReactionTypeEmoji.builder()
                                        .emoji(emoji)
                                        .build()
                        ))
                        .build());
    ...
    public void answerCallback(String text, boolean showAlert) {
        if (update.getCallbackQuery() != null) {
            try {
                client.execute(pro.kaleert.nyagram.api.methods.AnswerCallbackQuery.builder()
    ...
    public CompletableFuture<Serializable> editMessage(String text, ReplyKeyboard replyMarkup) {
        ...
        pro.kaleert.nyagram.api.objects.message.MaybeInaccessibleMessage maybeMsg = update.getCallbackQuery().getMessage();
        boolean hasMedia = false;
        if (maybeMsg instanceof pro.kaleert.nyagram.api.objects.message.Message msg) {
            hasMedia = msg.hasMedia();
        }

        if (hasMedia) {
            return client.executeAsync(pro.kaleert.nyagram.api.methods.updatingmessages.EditMessageCaption.builder()
            ...
            return client.executeAsync(pro.kaleert.nyagram.api.methods.updatingmessages.EditMessageText.builder()
                    .chatId(chatId.toString())
                    .messageId(msgId)
                    .text(text)
                    .parseMode("HTML")
    ...
    public CompletableFuture<Serializable> editMedia(InputMedia media, ReplyKeyboard replyMarkup) {
        ...
        return client.executeAsync(pro.kaleert.nyagram.api.methods.updatingmessages.EditMessageMedia.builder()
                .chatId(chatId.toString())
                .messageId(msgId)
                .media(media)
                .replyMarkup((InlineKeyboardMarkup) replyMarkup)
